...
export interface PartnerWorkspaceState
{
  workspaceId: string
  role: 'owner' | 'partner'
...
/** Carrega o workspace de casal do usuário logado (se houver) */
...
/** Garante workspace + membership owner e devolve link de convite */
export async function createPartnerInvite(): Promise<{ code: string; url: string } | null>
{
  ...
  else
  {
    ...
    if (wsErr || !ws)
    {
      console.error('createPartnerInvite workspace:', wsErr)
      return null
    }
    ...
    if (memErr)
    {
      console.error('createPartnerInvite member:', memErr)
      return null
    }
  }
  ...
  if (error)
  {
    console.error('createPartnerInvite invite:', error)
    return null
  }

  const base = typeof window !== 'undefined' ? window.location.origin : ''
  return { code, url: `${base}/parceiro/${code}` }
}
...
export async function leavePartnerWorkspace(): Promise<{ ok: boolean; message: string }>
{
  const uid = (await supabase.auth.getUser()).data.user?.id
  if (!uid) return { ok: false, message: 'Faça login' }
  ...
  if (error)
  {
    return { ok: false, message: 'Não foi possível sair do workspace' }
  }

  return { ok: true, message: 'Você saiu do workspace de casal' }
}
