...
})
{
  ...
  return (
    <div className={`grid grid-cols-[200px_1fr] gap-3 items-center min-h-[40px] border-b border-line ${AXEL_ROW_HOVER}`}>
      <button
        type="button"
        onClick={onOpen}
        className={`text-left text-xs truncate px-3 py-2 ${AXEL_TEXT_SECONDARY} hover:text-ink`}
      >
      ...
      <div className="relative h-8 py-1.5 pr-3">
        ...
        >
          <div
            className={`h-full ${AXEL_PROGRESS} opacity-80 transition-all duration-500`}
            style={{ width: `${percent}%` }}
          />
          <span className={`absolute inset-0 flex items-center px-2 text-[10px] truncate pointer-events-none ${AXEL_TEXT_PRIMARY}`}>
            {cleanTitleForDisplay(tarefa.titulo)}
          </span>
        </button>
      </div>
...
export function AxelKanbanTimelineView({ tarefas, onOpen }: AxelKanbanTimelineViewProps)
{
  ...
  return (
    <div className={`flex-1 min-h-[480px] ${AXEL_KANBAN_TABLE} flex flex-col`}>
      <div className="grid grid-cols-[200px_1fr] gap-3 border-b border-line bg-chrome/50 px-2 py-2 shrink-0">
        <span className={`font-mono text-[9px] uppercase tracking-[0.14em] pl-2 ${AXEL_TEXT_SECONDARY}`}>
          Tarefa
        </span>
        <div
          className={`grid font-mono text-[9px] tabular-nums pr-3 ${AXEL_TEXT_SECONDARY}`}
      ...
      >
        ...
        {sorted.length === 0 && (
          <p className={`text-sm text-center py-16 ${AXEL_TEXT_SECONDARY}`}>
