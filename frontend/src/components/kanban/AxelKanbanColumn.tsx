...
}: AxelKanbanColumnProps)
{
  ...
  const shellClass = embedded
    ? `${AXEL_KANBAN_COLUMN_EMBEDDED} ${AXEL_KANBAN_COLUMN_DIVIDER} ${isOver ? 'bg-accent-muted/20' : ''} ${emphasized ? 'bg-chrome/25' : ''}`
  ...
  return (
    ...
    >
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <h2 className={`font-mono text-[10px] uppercase tracking-[0.14em] truncate ${AXEL_TEXT_PRIMARY}`}>
                {title}
              </h2>
              <span
      ...
      >
        {isEmpty ? (
          <div
            className={`flex flex-col items-center justify-center flex-1 min-h-[120px] px-4 py-8 text-center ${AXEL_KANBAN_DROPZONE} ${
          ...
          >
            ...
            <p className={`text-[11px] mt-1.5 max-w-[200px] ${AXEL_TEXT_SECONDARY}`}>
