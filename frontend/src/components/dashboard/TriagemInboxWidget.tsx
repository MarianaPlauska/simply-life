    ...
    <section className="bg-card border border-zinc-900 rounded-md">
      <header className="px-3 py-2 border-b border-zinc-900 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-[11px] font-bold text-violet-300 uppercase tracking-widest">Inbox IA</h2>
          ...
          {events.slice(0, 6).map((event) =>
          {
            ...
            return (
              ...
              >
                ...
                <span className="text-[11px] text-zinc-400 shrink-0 truncate max-w-[110px]">{event.sender}</span>
                ...
                {(event.keywords_detectadas || []).slice(0, 1).map((kw) => (
                  <span key={kw} className="hidden md:inline text-[10px] text-violet-400 truncate max-w-[80px]">
                    #{kw}
                  </span>
                ))}
                ...
                <span className={`text-[11px] font-mono font-semibold ${getUrgencyColor(event.score_urgencia)} shrink-0`}>
