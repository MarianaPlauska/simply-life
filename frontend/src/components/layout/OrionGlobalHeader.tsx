...
import { countUrgentDeadlines } from '../../lib/orionAlerts'
import { useNavigate } from 'react-router-dom'
import { OrionStreakPopover } from './OrionStreakPopover'
import {
...
// Header global ORION — navegação, notificações e acessibilidade em todas as páginas
...
export function OrionGlobalHeader()
{
  ...
  return (
    <header className={`shrink-0 w-full border-b border-line relative z-50 ${ORION_CHROME_PLANE}`}>
      <div className="px-4 md:px-6 lg:px-8 h-14 flex items-center justify-between gap-4">
        <div className="flex items-center gap-2 shrink-0 min-w-0">
          <span className={`font-mono text-[11px] uppercase tracking-wider hidden sm:inline ${ORION_TEXT_SECONDARY}`}>Simply-Life</span>
          <span className="text-ink-muted hidden sm:inline">/</span>
          <span className={`text-[14px] font-display truncate ${ORION_TEXT_PRIMARY}`}>
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          <div className="hidden lg:flex items-center gap-2 bg-chrome border border-line rounded-sl px-3 py-1.5">
            ...
            <span className={`font-mono text-[11px] ${ORION_TEXT_SECONDARY}`}>Buscar</span>
            ...
            {isNotifOpen && (
              <div className="absolute right-0 top-10 w-80 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-xl overflow-hidden z-[100]">
                <div className="flex items-center justify-between px-4 py-2.5 border-b border-zinc-200 dark:border-zinc-800">
                  <span className={`text-[12px] font-semibold ${ORION_TEXT_PRIMARY}`}>Notificações</span>
                  {unreadCount > 0 && (
                    <button
                      type="button"
                ...
                <div className="max-h-72 overflow-y-auto">
                  {notificacoes.length === 0 ? (
                    <div className={`px-4 py-6 text-center text-[12px] ${ORION_TEXT_SECONDARY}`}>Nenhuma notificação</div>
                  ) : (
                        ...
                        >
                          ...
                          <div className="flex items-start gap-3 px-3.5 py-2.5 flex-1 min-w-0">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className={`text-[12px] truncate ${!n.lida ? `font-medium ${ORION_TEXT_PRIMARY}` : ORION_TEXT_SECONDARY}`}>
                                {n.titulo}
                              </div>
                              {n.mensagem && (
                                <div className={`text-[11px] truncate ${ORION_TEXT_SECONDARY}`}>{n.mensagem}</div>
                              )}
                            </div>
                          </div>
                        </button>
          ...
          <div ref={profileRef} className="relative z-[100]">
            ...
            {isProfileOpen && (
              <div className="absolute right-0 top-10 w-44 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-xl overflow-hidden z-[100]">
                <button
                  type="button"
                  className={`w-full flex items-center gap-2.5 px-3.5 py-2.5 text-[12px] hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors ${ORION_TEXT_PRIMARY}`}
