...
interface UseAxelIngestionOptions
{
  enabled?: boolean
}

export function useAxelIngestion({ enabled = true }: UseAxelIngestionOptions = {})
{
  ...
  const setAxelIngestionPolling = useTaskStore((s) => s.setAxelIngestionPolling)
  const highlightIds = useTaskStore((s) => s.axelIngestionHighlightIds)
